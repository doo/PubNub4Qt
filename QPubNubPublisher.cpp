...
QPubNubPublisher::QPubNubPublisher(
...
#ifdef Q_PUBNUB_CRYPT
void QPubNubPublisher::encryptMessages(const QByteArray& cipherKey) {
  m_cipherKey = QCryptographicHash::hash(cipherKey, QCryptographicHash::Sha256).toHex();
}
#endif

void QPubNubPublisher::signMessages(const QByteArray& secretKey) {
  m_secretKey = secretKey;
}

void QPubNubPublisher::publish(const QJsonValue& value) {
...
QByteArray QPubNubPublisher::toByteArray(const QJsonValue& value) {
  switch (value.type()) {
...
QByteArray QPubNubPublisher::signature(const QByteArray& message) {
...
QString QPubNubPublisher::url(const QByteArray& message) {
...
void QPubNubPublisher::onError(QNetworkReply::NetworkError code) {
  auto reply = qobject_cast<QNetworkReply*>(sender());
  reply->deleteLater();
  emit error(reply->errorString());
}

void QPubNubPublisher::readyRead() {
  auto reply = qobject_cast<QNetworkReply*>(sender());    
